...
class SupportMessage(models.Model):
    ...
    conversation = models.ForeignKey(
        'help_center.SupportConversation',
    ...
    def __str__(self):
        return f"Support Message #{self.message_id} (Conversation #{self.conversation_id})"
    ...
    def get_all(conversation_id: int, sort_order: str = 'asc') -> list:
        data = SupportMessage.objects.filter(conversation_id=conversation_id)
        data = data.order_by(('-' if sort_order == 'desc' else '') + 'created_at', 'message_id')
        return list(data.values(*SupportMessage.VALUES_FIELDS))
