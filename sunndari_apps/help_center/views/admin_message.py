...
class AdminMessageView:
    ...
    def get_all_extract(self, params: AdminGetMessagesRequest):
        SupportChatService.get_conversation_for_admin(conversation_id=params.conversation_id, user_id=params.user_id)
    ...
    def create_extract(self, params: AdminCreateMessageRequest):
        message = SupportChatService.send_admin_message(
            conversation_id=params.conversation_id, admin_user_id=params.user_id, content=params.content,
        )
        utils = HelpCenterUtils(entity='message')
        data = json.loads(utils.mapper([message]))[0]
        return Response(
            status=status.HTTP_201_CREATED,
            data=Utils.success_response_data(message='Reply sent successfully', data=data)
        )
