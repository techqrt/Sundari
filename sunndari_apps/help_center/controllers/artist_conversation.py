...
class ArtistConversationController:
    """Same underlying conversation logic as ConversationController — the
    support model has no role restriction, an artist's JWT works identically
    to a customer's. Kept as a separate controller/route purely so the artist
    app has its own documented, versionable API surface."""

    @extend_schema(
        description=(
            "Get the authenticated artist's current Help Center support conversation "
