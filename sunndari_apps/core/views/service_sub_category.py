...
class ServiceSubCategoryView:
    def __init__(self):
        self.data_get = Constants.data_get
        self.data_no_match = Constants.data_no_match

    @Common(response_handler=ServiceSubCategoryResponseGetSerializer).exception_handler
    def get_extract(self, params):
        sub_cat = ServiceSubCategory.get(sub_category_id=params.sub_category_id)
        if not sub_cat:
            raise ValueError(self.data_no_match)
        utils = CoreUtils(entity='service_sub_category', columns_required=[c for c in params.values.split(',') if c])
        data = json.loads(utils.mapper([sub_cat]))[0]
        return Response(
